...
export default {
  ...
  setup() {
    onMounted(() => {
      ...
      const $mapv = internalInstance.appContext.config.globalProperties.$mapv;
      const $initMap = internalInstance.appContext.config.globalProperties.$initMap;
      const $mapvgl = internalInstance.appContext.config.globalProperties.$mapvgl;
      const $purpleStyle =
        internalInstance.appContext.config.globalProperties.$purpleStyle;
      const $echarts = internalInstance.appContext.config.globalProperties.$echarts;
      const $darkStyle = internalInstance.appContext.config.globalProperties.$darkStyle;
      const $bmap2 = internalInstance.appContext.config.globalProperties.$bmap2;
      // 初始化百度地图
      const initBMap = () => {
        const map = $initMap({
          tilt: 0,
